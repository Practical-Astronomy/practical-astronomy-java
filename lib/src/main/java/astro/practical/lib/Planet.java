...
public class Planet {
    ...
    public PlanetPosition approximatePositionOfPlanet(double lctHour, double lctMin, double lctSec,
        ...
        int gdateMonth = Macros.localCivilTimeGreenwichMonth(lctHour, lctMin, lctSec, daylightSaving,
                zoneCorrectionHours, localDateDay, localDateMonth, localDateYear);
        int gdateYear = Macros.localCivilTimeGreenwichYear(lctHour, lctMin, lctSec, daylightSaving, zoneCorrectionHours,
        ...
        double utHours = Macros.localCivilTimeToUniversalTime(lctHour, lctMin, lctSec, daylightSaving,
                zoneCorrectionHours, localDateDay, localDateMonth, localDateYear);
        double dDays = Macros.civilDateToJulianDate(gdateDay + (utHours / 24), gdateMonth, gdateYear)
                - Macros.civilDateToJulianDate(0, 1, 2010);
        ...
        double ldDeg = Macros.wToDegrees(Math.atan2(y, x)) + planetData.node_LongitudeAscendingNode;
        ...
        double lamdaDeg1 = (rdAU < 1) ? 180 + leDeg2 + Macros.wToDegrees(aRad) : Macros.wToDegrees(aRad) + ldDeg;
        double lamdaDeg2 = lamdaDeg1 - 360 * Math.floor(lamdaDeg1 / 360);
        double betaDeg = Macros.wToDegrees(Math.atan(
                rdAU * Math.tan(psiRad) * Math.sin(Math.toRadians(lamdaDeg2 - ldDeg)) / (rAU2 * Math.sin(-leLdRad))));
        double raHours = Macros.decimalDegreesToDegreeHours(
                Macros.ecRA(lamdaDeg2, 0, 0, betaDeg, 0, 0, gdateDay, gdateMonth, gdateYear));
        double decDeg = Macros.ecDec(lamdaDeg2, 0, 0, betaDeg, 0, 0, gdateDay, gdateMonth, gdateYear);

        int planetRAHour = Macros.decimalHoursHour(raHours);
        int planetRAMin = Macros.decimalHoursMinute(raHours);
        double planetRASec = Macros.decimalHoursSecond(raHours);
        double planetDecDeg = Macros.decimalDegreesDegrees(decDeg);
        double planetDecMin = Macros.decimalDegreesMinutes(decDeg);
        double planetDecSec = Macros.decimalDegreesSeconds(decDeg);
    ...
    public PlanetPosition precisePositionOfPlanet(double lctHour, double lctMin, double lctSec,
        ...
        PlanetCoordinates coordinateResults = Macros.planetCoordinates(lctHour, lctMin, lctSec, daylightSaving,
                zoneCorrectionHours, localDateDay, localDateMonth, localDateYear, planetName);

        double planetRAHours = Macros.decimalDegreesToDegreeHours(Macros.ecRA(coordinateResults.planetLongitude, 0, 0,
                coordinateResults.planetLatitude, 0, 0, localDateDay, localDateMonth, localDateYear));
        double planetDecDeg1 = Macros.ecDec(coordinateResults.planetLongitude, 0, 0, coordinateResults.planetLatitude,
                0, 0, localDateDay, localDateMonth, localDateYear);

        int planetRAHour = Macros.decimalHoursHour(planetRAHours);
        int planetRAMin = Macros.decimalHoursMinute(planetRAHours);
        double planetRASec = Macros.decimalHoursSecond(planetRAHours);
        double planetDecDeg = Macros.decimalDegreesDegrees(planetDecDeg1);
        double planetDecMin = Macros.decimalDegreesMinutes(planetDecDeg1);
        double planetDecSec = Macros.decimalDegreesSeconds(planetDecDeg1);
    ...
    public VisualAspectsOfAPlanet visualAspectsOfAPlanet(double lctHour, double lctMin, double lctSec,
        ...
        PlanetCoordinates planetCoordInfo = Macros.planetCoordinates(lctHour, lctMin, lctSec, daylightSaving,
                zoneCorrectionHours, localDateDay, localDateMonth, localDateYear, planetName);

        double planetRARad = Math.toRadians(Macros.ecRA(planetCoordInfo.planetLongitude, 0, 0,
                planetCoordInfo.planetLatitude, 0, 0, localDateDay, localDateMonth, localDateYear));
        double planetDecRad = Math.toRadians(Macros.ecDec(planetCoordInfo.planetLongitude, 0, 0,
                planetCoordInfo.planetLatitude, 0, 0, localDateDay, localDateMonth, localDateYear));
        ...
        double phase1 = 0.5
                * (1.0 + Math.cos(Math.toRadians((planetCoordInfo.planetLongitude - planetCoordInfo.planetHLong1))));

        double sunEclLongDeg = Macros.sunLong(lctHour, lctMin, lctSec, daylightSaving, zoneCorrectionHours,
                localDateDay, localDateMonth, localDateYear);
        double sunRARad = Math.toRadians(
                Macros.ecRA(sunEclLongDeg, 0, 0, 0, 0, 0, greenwichDateDay, greenwichDateMonth, greenwichDateYear));
        double sunDecRad = Math.toRadians(
                Macros.ecDec(sunEclLongDeg, 0, 0, 0, 0, 0, greenwichDateDay, greenwichDateMonth, greenwichDateYear));
        ...
        double distanceAU = Util.round(planetCoordInfo.planetDistanceAU, 5);
        double angDiaArcsec = Util.round(angularDiameterArcsec, 1);
        double phase = Util.round(phase1, 2);
        int lightTimeHour = Macros.decimalHoursHour(lightTravelTimeHours);
        int lightTimeMinutes = Macros.decimalHoursMinute(lightTravelTimeHours);
        double lightTimeSeconds = Macros.decimalHoursSecond(lightTravelTimeHours);
        double posAngleBrightLimbDeg = Util.round(chiDeg, 1);
        double approximateMagnitude = Util.round(approximateMagnitude1, 1);
